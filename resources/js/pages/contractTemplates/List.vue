...
import type { Paginated, Column, SortKey, SortDir, ContractTemplate } from '@/types'
...
const props = defineProps<Props<ContractTemplate>>()
const { t } = useI18n()
const columns: Column<ContractTemplate>[] = [
  { key: 'name', label: t('contract_templates.name'), sortable: true },
]
...
<template>
  <AppLayout>
    <LayoutBasic :icon="Settings" :title="t('contract_templates.title')">
      ...
      >
        <template #headerActions>
          <Link :href="route('contract-templates.create')"><Button as="span"><Plus class="size-4" />{{ t('contract_templates.create') }}</Button></Link>
        </template>
        <template #headerFilters>
          <div class="flex gap-4">
            <InputText variant="filter" type="text" v-model="searchFilter" :placeholder="t('contract_templates.search_placeholder')" />
          </div>
        </template>
        <template #empty>
